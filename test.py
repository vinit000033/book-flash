...
class AdvancedMentionBot:
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        help_text += (
            "✨ *SMART FEATURES:*\n"
            "📚 **Book Suggestions** - Mention any book name and I'll suggest it if available\n"
            "💬 **Smart Replies** - I respond to greetings and book-related keywords\n"
            "🔍 **Book Search** - I can find books using fuzzy matching\n"
        ...
        help_text += (
            ...
            "• Mention a book name - I'll suggest if I have it\n"
            "• Say 'recommend books' - I'll help you find books\n"
            f"• Mention @{ADMIN_USERNAME} - Admin gets notified\n\n"
        )
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat.type in [Chat.GROUP, Chat.SUPERGROUP]:
            ...
            # Check for book mentions and suggestions
            words = text.split()
            for word in words:
                if len(word) > 3:  # Only check words longer than 3 characters
                    similar_books = self.find_similar_books(word, threshold=0.7)
                    if similar_books:
                        book = similar_books[0]
                        suggestion_text = (
                            f"📚 Did you mention *{book['name']}*?\n"
